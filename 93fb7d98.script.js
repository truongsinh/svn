...
window.app.controller('order', ['$scope', '$window', 'notification', 'socket', 'onUserLogin', function ($scope, window, notification, socket, onUserLogin) {
  ...
  $scope.submitOrder = function () {
    ...
    if (time.valueOf() === -62167226400000) {
      errorSet.push('Vui lòng điền ngày giao hàng');
    }
    else {
      // @todo: check process day
      if (time < now) {
        errorSet.push('Bạn đã đặt hàng ngày ' + time.format('DD/MM/YYYY') + ' trong quá khứ');
      }
      if (time > threeMonthLater) {
        errorSet.push('Hệ thống chưa hỗ trợ đơn đặt hàng hơn 3 tháng, vui lòng liên hệ trực tiếp!');
      }
      else {
    ...
    if ($scope.place) {
      ...
      errorSet.push('Vui lòng điền nơi giao hàng');
    ...
    if (itemSet.length > 0) {
      ...
      errorSet.push('Chưa có món được chọn');
    }
    ...
    if (errorSet.length > 0) {
      ...
      socketUser.emit('order:create', data)
        .spread(function (data) {
          var m = 'Đặt hàng thành công. Vui lòng chuyển khoản đến'
...
angular.module("savouryApp").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("template/order.html",
    ...
    "          <li>{{productMap[dish.id].name || '[Tên món]'}}</li>" +
    "          <li>{{dish.quantity || '[số lượng]'}} {{productMap[dish.id].unit || '[đơn vị tính]'}}</li>" +
    "          <li>{{ productMap[dish.id].totalPrice(dish.quantity) || '[giá thành]' }}€</li>" +
    "        </ul>" +
    ...
    "        <span>{{productMap[dish.id].unit || \"Đơn vị tính\"}}</span>" +
    "      </label>" +
